...
```{r}
##This does not take as long as the raster() package and fasterize is no longer needed - uses the extent, proj, etc of an existing raster layer
...
class(homerangeRD)
plot(homerangeRD, col=2:4)  ##95% of kernel home range 
...
Checking areas, etc 
```{r, warnings = FALSE}
#check area for both wolf packs
...
## this UD is dominated by bow valley data because there is way more - must do ud for ecah unit and then add them
```
...
crs(disthumanaccess2) <- "+proj=utm +zone=11 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"
...
##### these reflect differences in sample sizee, don't look at height
